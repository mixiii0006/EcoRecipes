  ...
  <div class="input-ingredients">
    ...
    <main class="main-content">
      ...
      <div class="input-section">
        ...
        <div v-if="totalCarbon" class="carbon-box">
          <span class="label">Total Carbon:</span>
          <span class="value">{{ totalCarbon.toFixed(2) }} CO₂eq/kg</span>
        </div>
      </div>

      <!-- Carbon Explanation Notes -->
      <div class="carbon-legend-box">
        <ul>
          <li><strong>Carbon Avoided</strong>: The carbon footprint you save by using your existing ingredients to cook this recipe.</li>
          <li><strong>Carbon to Emit</strong>: The additional carbon footprint generated by the new ingredients you need to buy.</li>
          <li><strong>Efficiency</strong>: The ratio between avoided carbon and carbon to emit — a higher percentage means greener cooking.</li>
        </ul>
      </div>
            ...
            <div class="recipe-grid">
              ...
              <div v-else v-for="(rec, index) in recommendations" :key="index" class="card-link">
                <RecipeCard
                  ...
                  :image="normalizeImagePath(rec.image || rec.Image_Name) || 'https://…'"
                  :name="rec.title || rec.title_cleaned || rec.name || 'No Title'"
                  :favorites="model.favorites"
                  :cooks="model.cooks"
                  :carbon="rec.carbon"
                  :totalRecipeCarbon="rec.totalRecipeCarbon"
                  :totalUsedCarbon="rec.totalUsedCarbon"
                  :totalMissingCarbon="rec.totalMissingCarbon"
...
export default {
  ...
  methods: {
    ...
    async submitIngredients() {
      await this.presenter.submitIngredients();
      this.update();

      console.log("📦 Full recommendation data after update:");
      this.recommendations.forEach((rec, idx) => {
        console.log(`Recipe ${idx}:`);
        console.log("  name:", rec.title || rec.name);
        console.log("  total_recipe_carbon:", rec.total_recipe_carbon);
        console.log("  total_used_carbon:", rec.total_used_carbon);
        console.log("  total_missing_carbon:", rec.total_missing_carbon); // ✅ ini penting
    ...
    update() {
      this.ingredients = this.model.ingredients;
      this.recommendations = this.model.recommendations;
      console.log("Updated recommendations in inputView:", this.recommendations);
      this.totalCarbon = this.model.totalCarbon;
      this.recentSearches = this.model.recentSearches;
      this.loading = this.model.loading;
      this.showModal = this.model.showModal;
